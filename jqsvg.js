(function () {
    var ul; // Declare a variable to store the <ul> element
    ...
    function SVG3DTagCloud(element, params) {
        ...
        function addEntry(index, entryObj, x, y, z) {
            ...
            // If ul is not created yet, create it
            if (!ul) {
            ...
            entry.listItem = document.createElement('li');

            // Appending the entry.element to the list item
            entry.listItem.appendChild(entry.element);

            // Appending the list item to the <ul> element
            ul.appendChild(entry.listItem);
